...
/*
	...
	- 디테일에 Instance Synced는 블랙보드끼리 같은 값을 같게 됨(정적 멤버 변수의 느낌)
	- UObject의 자식 클래스들은 Object 타입을 주고 밑에서 다시 세부 설정하게 만들어져 있음
	- Parent 노드도 줄 수 있는데 Parent에서 작성한 키에 추가로 더 만들어서 쓸 수 있음(상속 가능)
	...
	- ReveiceActivationAI는 service를 달아준 노드가 활성화 되면 실행된다고 보면 됨
	...
	+ Nav Mesh Bound Volume이 있어야 <길찾기> 가능 -> P를 누르면 갈 수 있는 길이 보이고 Volume의 Z Scale도 중요함
	...
	- BTT는 Excute이벤트가 끝나는 게 아니라 <FinishExecute를 실행>해서 성공이든 실패든 알려주어야 노드 탈출 가능함
